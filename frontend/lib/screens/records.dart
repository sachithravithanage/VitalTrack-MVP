...
class _RecordsListScreenState extends State<RecordsListScreen> {
  ...
  Future<void> _exportPdf(AppState app, List<RecordEntry> records) async {
    try {
      String pdfUrl = '';

      try {
        pdfUrl = await app.exportRecordsPdf(
          filter: _filter,
          patientId: widget.patientId,
        );
      } catch (error) {
        if (!_isStepUpRequiredError(error)) {
          rethrow;
        }

        final String? stepUpToken = await _requestExportStepUpToken(app);
  ...
  bool _isStepUpRequiredError(Object error) {
    ...
    if (responseData is Map<String, dynamic>) {
      ...
      if (errorObj is Map<String, dynamic>) {
        final String code = (errorObj['code'] ?? '').toString().toLowerCase();
        return code == 'step_up_required' || code == 'step_up_invalid';
      }
    }
  ...
  Future<String?> _requestExportStepUpToken(AppState app) async {
    String? stepUpToken;
    String? devOtp;

    try {
      final response = await app.sendStepUpOtp(purpose: 'export_records');
        ...
        await Navigator.of(context).push<bool>(
          MaterialPageRoute<bool>(
            builder: (_) => OtpVerificationScreen(
              ...
              subtitle: app.t('enter_otp_sent_mobile'),
              ...
              onResendOtp: () async {
                final response = await app.sendStepUpOtp(
                  purpose: 'export_records',
                );
                devOtp = response['otp']?.toString();
              },
