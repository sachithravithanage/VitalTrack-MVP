...
const String _sessionAuthTokenKey = 'session_auth_token';
...
class ApiClient {
  ...
  static String _getBaseUrl() {
    ...
    final String configuredBaseUrl = fromDefine.trim();

    if (!kDebugMode) {
      if (configuredBaseUrl.isEmpty) {
        throw StateError(
          'API_BASE_URL is required for release builds. '
          'Use --dart-define=API_BASE_URL=https://your-api-host.',
        );
      }

      final Uri? parsed = Uri.tryParse(configuredBaseUrl);
      final String host = parsed?.host.toLowerCase() ?? '';
      const Set<String> disallowedHosts = {
        'localhost',
        '127.0.0.1',
        '0.0.0.0',
        '10.0.2.2',
      ...
      if (disallowedHosts.contains(host)) {
        throw StateError(
          'API_BASE_URL for release builds must not use a local host. '
          'Current host: $host',
        );
      }
...
class _AuthInterceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    final token = storageService.getString(_sessionAuthTokenKey);
  ...
  Future<void> onError(
    ...
    if (err.response?.statusCode == 401) {
      await storageService.remove(_sessionAuthTokenKey);
