...
class PdfGenerator {
  ...
  static Future<void> generateAndShareReport({
    ...
    if (patient.dob.isNotEmpty) {
      try {
        final DateTime dob = DateTime.parse(patient.dob);
        final int years = DateTime.now().year - dob.year;
        age = '$years yrs';
      } catch (_) {}
    }
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          final List<pw.Widget> graphWidgets = [];
  ...
  static pw.Widget _buildGraph(
      ...
      pw.CustomPaint(
        ...
        painter: (PdfGraphics canvas, PdfPoint size) {
          ...
          for (int i = 0; i < dataPoints.length; i++) {
            ...
            final double y = normalizedY * size.y;
    ...
    for (int i = 0; i < dataPoints.length; i++) {
      ...
      final double y = normalizedY * chartHeight;

      final String valStr =
          val == val.toInt() ? val.toInt().toString() : val.toStringAsFixed(1);

      final bool isLastPoint =
          i == dataPoints.length - 1 && dataPoints.length > 1;
      final double textOffsetX = isLastPoint ? x - 12 : x - 6;
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: List.generate(5, (i) {
                final double val = maxVal - (range * (i / 4));
                final String labelStr = val == val.toInt()
                    ? val.toInt().toString()
      ...
      children: [
        pw.TableRow(
          ...
          children: headers.asMap().entries.map((entry) {
            ...
            final pw.Alignment align = (idx == 3 || idx == 4)
                ? pw.Alignment.center
        ...
        ...logs.map((item) {
          final log = item.log;
          final String unit = _getUnit(item.category, log.value1);
          String valueStr = '';
