...
class _HeatMapScreenState extends State<HeatMapScreen> {
  ...
  void _selectDistrictByName(String name) {
    ...
    final int foundIndex =
        data.indexWhere((d) => d.district.toLowerCase() == name.toLowerCase());
              ...
              child: Container(
                ...
                child: Autocomplete<String>(
                  optionsBuilder: (TextEditingValue textEditingValue) {
                    ...
                    return _districtsList.where((String option) => option
                        .toLowerCase()
            ...
            SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _buildChip(
                      'Leptospirosis', selectedDisease == 'Leptospirosis'),
                ],
              ),
            ),
  ...
  List<Widget> _buildDynamicCards() {
    ...
    if (_selectedDistrictIndex != -1 && _selectedDistrictIndex < data.length) {
      ...
      final List<DistrictData> sorted = List.from(data);
      sorted.sort((a, b) =>
          (selectedDisease == 'Dengue' ? b.dengueCases : b.leptoCases)
              .compareTo(
    ...
    return displayedData.map((d) {
      final double cases =
          (selectedDisease == 'Dengue' ? d.dengueCases : d.leptoCases);
  ...
  List<DistrictData> _getDistrictData() {
    return const [
      DistrictData('Colombo', 2800, 900),
      DistrictData('Gampaha', 1500, 250),
      DistrictData('Kalutara', 800, 400),
      DistrictData('Kandy', 950, 300),
      DistrictData('Matale', 400, 550),
      DistrictData('Nuwara Eliya', 200, 700),
      DistrictData('Galle', 300, 1100),
      DistrictData('Matara', 350, 350),
      DistrictData('Hambantota', 150, 500),
      DistrictData('Jaffna', 600, 450),
      DistrictData('Kilinochchi', 100, 60),
      DistrictData('Mannar', 80, 120),
      DistrictData('Vavuniya', 120, 80),
      DistrictData('Mullaitivu', 60, 100),
      DistrictData('Batticaloa', 450, 600),
      DistrictData('Ampara', 500, 150),
      DistrictData('Trincomalee', 350, 350),
      DistrictData('Kurunegala', 1100, 300),
      DistrictData('Puttalam', 700, 200),
      DistrictData('Anuradhapura', 550, 400),
      DistrictData('Polonnaruwa', 300, 950),
      DistrictData('Badulla', 400, 800),
      DistrictData('Monaragala', 250, 1500),
      DistrictData('Ratnapura', 900, 2800),
  ...
  Widget _buildDistrictCard(
      String title, String subtitle, String stats, Color color, IconData icon) {
    return Container(
...
class _HeatmapCardState extends State<HeatmapCard> {
  ...
  void _updateSource() {
    setState(() {
      _shapeSource = MapShapeSource.asset(
        ...
        shapeColorValueMapper: (int index) => widget.disease == 'Dengue'
            ? _data[index].dengueCases
            : _data[index].leptoCases,
        shapeColorMappers: widget.disease == 'Dengue'
            ? _getDengueMappers()
            : _getLeptoMappers(),
      );
    });
    ...
    return Container(
      ...
      child: Column(
        children: [
          Padding(
            padding: const EdgeInsets.all(16),
            child: Text('${widget.disease} Distribution Map',
                style:
                ...
                Padding(
                  ...
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment
                        .start, // Changed to start for Top Right
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: widget.disease == 'Dengue'
                        ? _buildDengueLegend()
                        : _buildLeptoLegend(),
                  ),
                ),
  ...
  List<MapColorMapper> _getLeptoMappers() => [
        const MapColorMapper(from: 0, to: 500, color: Color(0xFFFEE5D9)),
        const MapColorMapper(from: 501, to: 1500, color: Color(0xFFA0BCD8)),
        const MapColorMapper(from: 1501, to: 10000, color: Color(0xFF4175B5)),
      ];

  List<MapColorMapper> _getDengueMappers() => [
        const MapColorMapper(from: 0, to: 500, color: Color(0xFFFEE5D9)),
        const MapColorMapper(from: 501, to: 1500, color: Color(0xFFFB6A4A)),
        const MapColorMapper(from: 1501, to: 10000, color: Color(0xFFA50F15))
      ];
}
