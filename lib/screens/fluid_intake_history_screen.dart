...
class _FluidIntakeHistoryScreenState extends State<FluidIntakeHistoryScreen> {
  ...
  void _saveEntry() {
    String formattedVal = '${_currentIntake.toStringAsFixed(1)} L';

    bool isAlert = _currentIntake < 1.5;
    String status = isAlert ? 'LOW INTAKE' : 'ON TRACK';
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Fluid intake saved successfully!'),
          backgroundColor: Color(0xFF14B8A6)),
    );
  }

  List<double> _getTrendData() {
    List<double> chartData = List.filled(7, 1.5);
    if (globalFluidHistory.isNotEmpty) {
      var recentRecords = globalFluidHistory.take(7).toList().reversed.toList();
      int startIndex = 7 - recentRecords.length;
      for (int i = 0; i < recentRecords.length; i++) {
        String rawVal = recentRecords[i].value.replaceAll(' L', '').trim();
        chartData[startIndex + i] = double.tryParse(rawVal) ?? 1.5;
      }
  ...
  Widget build(BuildContext context) {
    final latestRecord = globalFluidHistory.isNotEmpty
        ...
        child: Column(
          ...
          children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Text(latestRecord.value.replaceAll(' L', ''),
                              style: GoogleFonts.nunito(
                                  fontSize: 32,
                  ...
                  SizedBox(
                    ...
                    child: CustomPaint(
                      painter: AngularTrendChartPainter(
                          ...
                          minY: 0.0,
                          maxY: 5.0,
                          lineColor: const Color(0xFF0EA5E9), // Tealish Blue
            ...
            if (globalFluidHistory.isEmpty)
              ...
              ...globalFluidHistory.map((record) => _buildDailyLogItem(record)),
          ],
        ),
      ),
    );
