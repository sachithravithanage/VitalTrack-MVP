...
class LeptoHealthDataHistoryScreen extends StatefulWidget {
  ...
  @override
  State<LeptoHealthDataHistoryScreen> createState() =>
      _LeptoHealthDataHistoryScreenState();
}

class _LeptoHealthDataHistoryScreenState
    extends State<LeptoHealthDataHistoryScreen> {
  ...
  void initState() {
    super.initState();
    _activeTab =
        _tabs.contains(widget.initialTab) ? widget.initialTab : _tabs[0];
  }
    ...
    if (!_stateInitialized) {
      ...
      if (data.inputType == InputType.doubleSlider) {
        ...
        for (var item in data.checklistItems!) {
          ...
          _checklistState[item['name']] =
              _checklistState[item['name']] ?? false;
        }
      }
  ...
  Widget _buildTabBar(
      LeptoMetricConfig data, LeptoHealthDataProvider provider) {
    return Container(
      color: Colors.white,
                  ...
                  children: [
                    Text(data.currentValue,
                        style: TextStyle(
                            fontSize: 24,
                            fontWeight: FontWeight.bold,
                            color: data.inputType == InputType.doubleSlider
                                ? data.baseColor
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                  data.inputType == InputType.checklist
                      ? 'New Symptom Entry'
                      : 'New Entry',
                  style: const TextStyle(
                      fontWeight: FontWeight.bold,
              ...
              if (data.inputType == InputType.singleSlider)
                Text('${_sliderValue1.toStringAsFixed(1)} ${data.unit}',
                    style: TextStyle(
            ...
            child: TextButton(
              onPressed: () {
                ...
                if (data.inputType == InputType.singleSlider) {
                  ...
                  final List<String> activeSymptoms = _checklistState.entries
                      .where((e) => e.value)
                      .map((e) => e.key)
                      .toList();
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('$_activeTab entry saved!'),
  ...
  Widget _buildDoubleSlider(LeptoMetricConfig data) {
    return Column(
      children: [
        _buildBPSliderRow('Systolic', _sliderValue1, data.sliderMin1,
            data.sliderMax1, (v) => setState(() => _sliderValue1 = v)),
        const SizedBox(height: 16),
        _buildBPSliderRow('Diastolic', _sliderValue2, data.sliderMin2!,
            data.sliderMax2!, (v) => setState(() => _sliderValue2 = v)),
      ],
    );
  }

  Widget _buildBPSliderRow(String label, double val, double min, double max,
  ...
  Widget _buildChecklist(LeptoMetricConfig data) {
    return Column(
      children: data.checklistItems!.map((item) {
        final bool isChecked = _checklistState[item['name']] ?? false;
        return CheckboxListTile(
          ...
          activeColor: const Color(0xFF14B8A6),
          onChanged: (bool? val) =>
              setState(() => _checklistState[item['name']] = val!),
        );
      }).toList(),
    );
  }
                ...
                if (data.chartType == ChartType.curve)
                  CustomPaint(
                      ...
                      painter: _CurvePainter(
                          color: data.baseColor, dataPoints: data.chartData)),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < dataPoints.length; i++) {
      final double y = size.height -
          ((dataPoints[i] - minVal) / range * size.height * 0.8 + 10);
